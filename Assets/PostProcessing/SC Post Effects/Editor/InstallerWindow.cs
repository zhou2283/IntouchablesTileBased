...
namespace SCPE
{
    ...
    {
        //Window properties
        private static readonly int width = 440;
        private static readonly int height = 525;
...
#if !SCPE
            EditorUtility.DisplayDialog("SC Post Effect", "Installation wasn't completed\n\nYou can re-open this wind through the Help menu", "Ok");
#endif
        }
        ...
        private void OnGUI()
        {
            ...
            Rect areaRect = new Rect(width / 2, height - 70, width / 2.1f, height - 25);
        ...
        private void StartScreen()
        {
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.textArea))
            {
                ...
                //Folder
                {
                    ...
                    string folderText = (Installer.IS_CORRECT_BASE_FOLDER) ? "Inside \"PostProcessing/\"" : "Outside \"PostProcessing/\"";
                ...
                string ppsText = (PostProcessingInstallation.IS_INSTALLED) ? "Installed" : "Not installed";
...
#endif
                SCPE_GUI.DrawStatusBox(new GUIContent(ppsLabel, EditorGUIUtility.IconContent("Camera Icon").image), ppsText, ppsStatus);
                ...
                if (PostProcessingInstallation.IS_INSTALLED == false)
                {
                    ...
                    {
#if !UNITY_2018_1_OR_NEWER
                        if (SCPE_GUI.DrawActionBox("Download", EditorGUIUtility.IconContent("BuildSettings.Standalone.Small").image))
                        {
                            Application.OpenURL(PostProcessingInstallation.PP_DOWNLOAD_URL);
                            if (EditorUtility.DisplayDialog("Post Processing Stack download", "Once the file has been downloaded, locate the file path to install it", "Browse"))
                            {
            ...
            {
#if !UNITY_2018_1_OR_NEWER
                hasError = (PostProcessingInstallation.PACKAGE_PATH == string.Empty) ? true : false;
#else
                hasError = !PostProcessingInstallation.IS_INSTALLED;
#endif
            }
        }
        ...
        private void InstallScreen()
        {
            ...
            if (Installer.CURRENTLY_INSTALLING || Installer.IS_INSTALLED)
            {
                ...
                {
                    //Rect barRet = new Rect(5, height - 115f, width - 10, 20f);
                    //SCPE_GUI.DrawProgressBar(barRet, Installer.INSTALL_PROGRESS);
                }
            }
