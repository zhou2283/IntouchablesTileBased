Shader "Hidden/SC Post Effects/Light Streaks"
{
	...
	{
		return LuminanceThreshold(SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoordStereo), _Threshold);

		float4 color = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoordStereo);

		float3 luminance = 1 - (color.rgb * 0.3 + color.rgb * 0.59 + color.rgb * 0.11);
		luminance.rgb = step(luminance.rgb, _Threshold.x);
